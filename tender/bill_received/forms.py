...
class LoanCollectionForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.fields['loan_info'] = forms.ModelChoiceField(queryset=LoanInformation.objects.filter(balance__gt=0))
    ...
    def clean(self):
        ...
        if total_amount > loan_obj.balance:
...
class LoanReceiveForm(forms.ModelForm):
    class Meta:
        model = MoneyReceived
        fields = ('loan_info','total_amount', 'received_method', 'account_no',
                'recieved_cheque_no', 'recieved_bank_name', 'bank_info', 'cash_balance', 'check_attachment')
